...
{{% notice info %}}
Most of the highlighting queries in the bundle are very basic, as they are copies of those included in the grammar repos. Queries for languages written in bold have received additional work to leverage more of the querying system's expressiveness.
...
Adding project-specific patterns through ~.dir-locals.el~:
#+begin_src emacs-lisp
...
When writing custom highlighting patterns, keep in mind that if a node matches multiple patterns in a highlighting query, earlier patterns are prioritized.
#+begin_src scheme
...
:END:
Functions and variables in this package have 2 prefixes:
- =tree-sitter-=: High-level features, i.e. the *apps*. For example, [[*Syntax Highlighting]].
- =ts-=: Core functionalities, i.e. the *lib*, which is the focus of this section.

[[https://tree-sitter.github.io/tree-sitter/using-parsers][Tree-sitter's own documentation]] is a good read to understand its concepts and features. This documentation focuses more on details that are specific to Emacs Lisp.
...
*** Data Types
- ~language~, ~parser~, ~tree~, ~node~, ~cursor~, ~query~, ~query-cursor~: corresponding Tree-sitter's data types, embedded in ~user-ptr~ objects.
...
{{% notice info %}}
These types are understood only by this package and its type-checking predicates, which are useful for debugging: ~ts-language-p~, ~ts-tree-p~, ~ts-node-p~... They are not recognized by ~type-of~.
{{% /notice %}}

For consistency with Emacs's conventions, there are some differences compared to Tree-Sitter's C/Rust APIs:
- *1-based* byte positions are used, instead of 0-based byte offsets.
- *1-based* line numbers are used, instead of 0-based row coordinates.
...
- ~ts-make-parser~ :: Create a new parser without setting a language.
- ~ts-set-language~ /~parser language~/ :: Set a parser's active language.
- ~ts-parse-string~ /~parser string~/ :: Parse a single string of source code. This is useful for quick, *one-off* parsing needs.
    #+begin_src emacs-lisp
      (let ((parser (ts-make-parser)))
        (ts-set-language parser (tree-sitter-require 'rust))
        (ts-parse-string parser "fn foo() {}"))
    #+end_src
- ~ts-parse-chunks~ /~parser input-function old-tree~/ :: Parse chunks of source code generated by an input function. /~input-function~/ should take 3 parameters: /~(bytepos line-number byte-column)~/, and return a fragment of the source code, starting from the position identified by either /~bytepos~/ or /~(line-number . byte-column)~/. It should return an empty string to signal the end of the source code.

    *Incremental parsing*: If you have already parsed an earlier version of this document, and it has since been edited, pass the previously parsed /~old-tree~/ so that its unchanged parts can be reused. This will save time and memory. For this to work correctly, you must have already edited it using ~ts-edit-tree~ function in a way that exactly matches the source code changes.
- ~ts-edit-tree~ /~tree ...~/ :: Prepare a tree for incremental parsing, by editing it to keep it in sync with source code that has been edited. You must describe the edit both in terms of byte positions and in terms of /~(line-number . byte-column)~/ coordinates.
...
The result of parsing is a syntax tree of the entire source code (string, buffer). It contains syntax nodes that indicate the structure of the source code. Tree-sitter provides APIs to inspect and [[*Walking][traverse]] this structure, but does not support modifying it directly (for source code transformation/generation purposes).

- ~ts-root-node~ /~tree~/ :: Get the root node of a syntax tree.
- ~ts-changed-ranges~ /~old-tree new-tree~/ :: Compare an edited old syntax tree to a newly parsed one. It is typically used in ~tree-sitter-after-change-functions~ hook. This function returns a sequence of ranges whose syntactic structure has changed. Each range is a vector in the form of /~[start-bytepos end-bytepos start-point end-point]~/.
...
- ~ts-tree-to-sexp~ :: <br>
- ~ts-node-to-sexp~ :: Debug utilities that return the sexp representation of a syntax tree/node, as a string.

*** Node Properties
Functions that return a node's property have the prefix =ts-node-=:
- ~ts-node-type~ :: <br>
...
- ~ts-node-extra-p~ :: Whether a node is an *extra node*, which is not required by the grammar, but can appear anywhere in the source code, like comments.
- ~ts-node-error-p~ :: Whether the node /represents/ a syntax error. The node type of an *error node* is the special symbol ~ERROR~.
- ~ts-node-has-error-p~ :: Whether the node /contains/ a syntax error.
- ~ts-node-missing-p~ :: Whether a node is a *missing node*, i.e. inserted by the parser in order to recover from certain kinds of syntax errors, like a missing semicolon.
- ~ts-node-start-byte~ :: <br>
- ~ts-node-end-byte~ :: The start/end byte position of a node.
- ~ts-node-start-position~ :: <br>
- ~ts-node-end-position~ :: The start/end position of a node.
- ~ts-node-range~ :: A node's /~[start-bytepos end-bytepos start-point end-point]~/.

*** Related Nodes
As described in the previous section, the ~-named-~  variants of the functions in this section allow working on the parse tree as an abstract syntax tree.

- ~ts-get-parent~ /~node~/ :: Get a node's parent node.
- ~ts-count-children~ /~node~/ :: <br>
- ~ts-count-named-children~ /~node~/ :: Count the number of child nodes (all, or named only).
- ~ts-get-nth-child~ /~node nth~/ :: <br>
- ~ts-get-nth-named-child~ /~node nth~/ :: Get a child node by its 0-based index (any, or named only).
    #+begin_src emacs-lisp
      (let ((func (tree-sitter-node-at-point 'function_item)))
        (ts-get-nth-child func 0)        ; An "fn" node
        (ts-get-nth-named-child func 0)) ; An 'identifier node
    #+end_src
- ~ts-get-child-by-field~ /~node field~/ :: Certain node types assign unique field names to specific child nodes. This function allows retrieving child nodes by their field names, instead of by their indexes. The field name should be specified as a keyword.
    #+begin_src emacs-lisp
      ;; Get name of the current function definition.
      (let ((func (tree-sitter-node-at-point 'function_item)))
        (ts-node-text (ts-get-child-by-field func :name)))
    #+end_src
...
- ~ts-get-next-sibling~ /~node~/ :: <br>
- ~ts-get-prev-sibling~ /~node~/ :: <br>
- ~ts-get-next-named-sibling~ /~node~/ :: <br>
- ~ts-get-prev-named-sibling~ /~node~/ :: Get next/previous sibling node (any, or named only).
- ~ts-get-descendant-for-position-range~ /~node beg end~/ :: <br>
- ~ts-get-named-descendant-for-position-range~ /~node beg end~/ :: Get smallest descendant node that spans the given range.
    #+begin_src emacs-lisp
      ;; Get the syntax node the cursor is on.
      (let ((p (point)))
        (ts-get-descendant-for-position-range
         (ts-root-node tree-sitter-tree) p p))
    #+end_src

# TODO: Explain node comparison and ts-node-eq
** Walking
:PROPERTIES:
...
- ~ts-make-cursor~ /~node-or-tree~/ :: Create a new cursor on a node. The cursor /cannot move out/ of this node. If called on a tree, the cursor is created on the tree's root node.
- ~ts-goto-parent~ /~cursor~/ :: <br>
- ~ts-goto-first-child~ /~cursor~/ :: <br>
- ~ts-goto-next-sibling~ /~cursor~/ :: Attempt to move the cursor to the parent node, the first child node, or the next sibling node. This function returns t if the move was successful, nil if the move is invalid.
- ~ts-goto-first-child-for-position~ /~cursor pos~/ :: Attempt to move the cursor to the first child node that extends beyond the given position. This function returns the index of the child node found, nil otherwise.
- ~ts-reset-cursor~ /~cursor node~/ :: Re-initialize the cursor to start on a different node.
- ~ts-current-node~ /~cursor~/ :: Get the node that the cursor is currently on.
- ~ts-current-field~ /~cursor~/ :: Get the field name (as a keyword) associated with the current node.
...
- ~ts-make-query~ /~language patterns [tag-assigner]~/ :: Create a new query for the given language. This query cannot be run on syntax nodes of other languages.
...
- ~ts-make-query-cursor~ :: Create a new *query cursor* to execute queries. It stores the state that is needed to iteratively search for matches.
- ~ts-query-captures~ /~query node text-function [query-cursor]~/ :: <br>
- ~ts-query-matches~ /~query node text-function [query-cursor]~/ :: Execute a query on the given syntax node. ~ts-query-captures~ returns a sequence of captures, sorted in the order they appear in the source code. ~ts-query-matches~ returns a sequence of matches, sorted in the order they were found.
