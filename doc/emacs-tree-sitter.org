...
For the full list of supported major modes, check the variable ~tree-sitter-major-mode-language-table~.
...
Syntax-aware language-agnostic mechanisms are meant to be defined by ~tree-sitter-mode~ and its dependent minor modes. They determine the language object to use by consulting the variable ~tree-sitter-major-mode-language-table~. This list is empty by default, and gets populated by ~tree-sitter-langs~ when it is loaded, and by language major modes that are ~tree-sitter~-aware.
...
- Variable: ~tree-sitter-major-mode-language-table~ :: @@html:<br>@@
