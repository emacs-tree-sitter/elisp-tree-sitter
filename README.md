...
One way to get these shared libraries is to use the `tree-sitter` CLI tool, or the wrapper script [`bin/ensure-lang`](bin/ensure-lang) (for languages official supported by the GitHub org [tree-sitter](https://github.com/tree-sitter/)).
...
- Enable `tree-sitter` in a major mode:
    ```emacs-lisp
    (require 'tree-sitter)
    ;;; Assuming ~/.tree-sitter/bin/rust.so was already generated by tree-sitter CLI tool.
    (add-hook 'rust-mode-hook #'tree-sitter-mode)
    ```
- Customize the language to used for a major mode:
    ```emacs-lisp
