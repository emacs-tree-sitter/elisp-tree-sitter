...
instead, to make this restoration exact."
  ...
  `(let* ((p (point))
          ...
          (steps (lts--node-steps old-node))
          (delta (- p (lts-node-start-position old-node)))
     ...
     (unwind-protect
         (save-excursion ,@body)
       (condition-case err
           (when-let ((node (lts--node-from-steps tree-sitter-tree steps)))
             (goto-char (+ delta (lts-node-start-position node)))
             (tree-sitter--recenter screen-line pixel-posn-y))
         (lts--invalid-node-step
          ,@(when tree-sitter-save-excursion-try-hard
              '((goto-char (lts-node-start-position (cadr err)))
                (tree-sitter--recenter screen-line pixel-posn-y))))))))
